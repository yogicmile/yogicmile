...
interface TierRate {
  ...
  symbol: string;
  name: string;
  rate: number; // paisa per 100 steps
...
export const useCoinRateSystem = () => {
  ...
  const tierRates: TierRate[] = [
    ...
    { tier: 3, symbol: '🎯', name: 'Token Phase', rate: 3, stepRequirement: 500000, timeLimit: 60, spiritualName: 'Strengthened Willpower' },
    { tier: 4, symbol: '💎', name: 'Gem Phase', rate: 5, stepRequirement: 750000, timeLimit: 75, spiritualName: 'Inner Clarity' },
    { tier: 5, symbol: '💠', name: 'Diamond Phase', rate: 7, stepRequirement: 1000000, timeLimit: 90, spiritualName: 'Unshakeable Focus' },
    { tier: 6, symbol: '👑', name: 'Crown Phase', rate: 10, stepRequirement: 1500000, timeLimit: 120, spiritualName: 'Mastery of Self' },
    { tier: 7, symbol: '⚡', name: 'Emperor Phase', rate: 15, stepRequirement: 2000000, timeLimit: 150, spiritualName: 'Transcendent Power' },
    { tier: 8, symbol: '🌟', name: 'Legend Phase', rate: 20, stepRequirement: 3000000, timeLimit: 200, spiritualName: 'Enlightened Being' },
    { tier: 9, symbol: '🔥', name: 'Immortal Phase', rate: 30, stepRequirement: 5000000, timeLimit: 300, spiritualName: 'Eternal Consciousness' }
  ];
  ...
  const calculateEarnings = useCallback((steps: number) => {
    ...
    return Math.floor((steps / 100) * effectiveRate);
  ...
  const checkTierProgression = useCallback(() => {
    ...
    if (currentSteps >= currentTierData.stepRequirement && currentTier < 9) {
      ...
      setCelebrationEvent({
        ...
        subMessage: `New rate: ${newTierData.rate} paisa per 100 steps`,
