-- Create spin wheel related tables for comprehensive tracking (with proper error handling)

-- Create spin_results table if not exists
...
-- Create spin_cooldowns table if not exists
...
-- Drop existing policies if they exist and recreate them
DROP POLICY IF EXISTS "Users can view their own spin cooldowns" ON public.spin_cooldowns;
DROP POLICY IF EXISTS "Users can manage their own spin cooldowns" ON public.spin_cooldowns;
