...
-- 2. Remove overly permissive user data policies and replace with privacy-respecting ones
DROP POLICY IF EXISTS "Users can view their own data" ON public.users;
DROP POLICY IF EXISTS "Public profiles are viewable by everyone" ON public.user_profiles;

-- Create proper user data access policies
...
-- 3. Fix user profiles with proper privacy controls
...
-- 5. Add privacy protection for forum posts
...
USING (
  ...
  EXISTS (
    ...
    WHERE up.user_id = forum_posts.author_id AND (
      (up.privacy_settings->>'profile_visibility')::text = 'public' OR
      (
        (up.privacy_settings->>'profile_visibility')::text = 'friends' AND
        EXISTS (
          SELECT 1 FROM public.friendships 
          WHERE status = 'accepted' AND 
...
-- 8. Create privacy settings table if it doesn't exist
...
-- 10. Add rate limiting for sensitive operations
